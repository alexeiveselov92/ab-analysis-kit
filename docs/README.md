...
6. **`abk dashboard`** (alias **`abk ui`**) serves the project-level view once you
   have more than a couple of experiments: one row per experiment (verdict, effect,
   sparkline), with buttons that spawn the very commands above as subprocesses, plus
   an editor that creates, edits and deletes the experiment YAML in place —
   validated at both levels before it lands, archived under `experiments/.history/`,
   and round-tripping your comments and layout verbatim. It **computes no statistic
   and takes no pipeline lock**: every verdict on the page is the readout's, and
   every job is a real `abk` subprocess doing the work in its own process. See
   [dashboard](guides/dashboard.md).

The full command surface is `init`, `init-claude`, `run`, `explore`, `dashboard`
(alias `ui`), `validate`, `plan`, `unlock`, `clean`, `verify-incremental`, and
`test-report`
...
- [The dashboard](guides/dashboard.md) — the project-level cockpit (`abk
  dashboard`, alias `abk ui`): one row per experiment, buttons that spawn `abk`
  commands, and creating / editing / deleting the experiment YAML in place.
