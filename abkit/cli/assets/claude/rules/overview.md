...
| `_ab_experiments` | Informational experiment catalog (descriptions, variants, the control arm, split, alpha, cadence, tags) for BI joins; the pipeline never reads it back for decisions. |
...
| `_ab_notify_states` | What `--notify` last ANNOUNCED per comparison (verdict + SRM flag + rollup identity + when), so an unchanged verdict is not re-sent on every scheduled run — plus one sentinel row per recurring signal (`metric='__stale__'`/`'__calibration_red__'`, empty arm pair) holding that condition's signature. Written only after a channel accepted the message; purged with the experiment by `abk clean --orphaned-experiments`, which resets the dedup. Not a BI table. |
| `_ab_unit_state` | Per-(unit, day) cumulative moments, written by the `state` pipeline stage for eligible closed-form metrics. Read on the compute path only with `compute.incremental_reads: true` — opt-in, and what `abk init` scaffolds, though the library default (the key absent) is off; any gap falls back to full recompute. Not a BI table. |
