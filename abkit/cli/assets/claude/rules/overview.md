...
| `_ab_notify_states` | What `abk run --notify` last ANNOUNCED per comparison (verdict + SRM flag + when), so an unchanged verdict is not re-sent on every scheduled run. Written only after a channel accepted the message; purged with the experiment by `abk clean --orphaned-experiments`, which resets the dedup. Not a BI table. |
