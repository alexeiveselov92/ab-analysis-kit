...
| `abk` commands, `--select`/`--metric`/`--method` selectors, `--from`/`--to`, `--full-refresh`, locks, `abk clean` | `.claude/rules/ab-analysis-kit/cli.md` |
| `abkit_project.yml`, `profiles.yml`, DB connections, statistical defaults, two-tier alpha | `.claude/rules/ab-analysis-kit/project.md` |
